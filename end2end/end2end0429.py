...
def create_model():
    ...
    embedding=tf.get_variable(name="embedding",initializer=tf.cast(vec,tf.float32),trainable=False)
    ...
    # l2_loss=tf.contrib.layers.apply_regularization(regularizer=tf.contrib.layers.l2_regularizer(0.5),weights_list=tf.trainable_variables())
    # loss=loss_+l2_loss
