...
def process(examples,max_seq_len):
    ...
    for example in examples:
        score_matrix = np.zeros([max_seq_len,max_seq_len],dtype=int)
        text=example["text"]
        l=len(text)
...
def get_spo_list(token,score_matrix):
    ...
    for ids in range(len(token)):
        ...
        if sum(cor_relation)>0:
            for j in range(len(token)):
                rel = id2raltion[str(cor_relation[j])]  # 两个字符之间对应的关系
                rel_set.add(rel)
...
def create_model():
    ...
    lstm_output=inp
    # left=Dense(units=256,activation="relu")(lstm_output)
    # right=Dense(units=256,activation="relu")(lstm_output)
    left=Conv1D(filters=64,kernel_size=5,activation="relu",padding="same")(lstm_output)
    right=Conv1D(filters=64,kernel_size=5,activation="relu",padding="same")(lstm_output)
    out=broadcasting(left,right)

    score=Dense(units=1,activation="relu")(out)
    pred_matrix=tf.reshape(score,[-1,max_seq_len,max_seq_len])
    pred_relation=tf.cast(tf.round(pred_matrix),tf.int32)
